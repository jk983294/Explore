"""
Adapted from Yahoo! Finance API.
Original author: Lukasz Banasiak
"""
...
# use for fast retrieval of data.
...
class FinanceDataStore(object):
    ...
    def __init__(self, db_full_path):
        ...
        """
        self.con = sqlite3.connect(db_full_path)
    ...
    def setup_db_for_hist_prices_storage(self, stock_sym_list):
        ...
        histdata_extr.set_interval_to_retrieve(
    ...
    def retrieve_hist_data_fr_db(self, stock_list=[], select_all=1):
        ...
        command_str = "SELECT * FROM %s where symbol in (%s)" % (
            self.hist_data_tablename, stock_sym_str)
        if select_all:
        ...
        self.hist_price_df = pandas.DataFrame(rows, columns=headers)

        # dividend data extract
        command_str = "SELECT * FROM %s where symbol in (%s)" % (
            self.divdnt_data_tablename, stock_sym_str)
        if select_all:
        ...
        rows = self.cur.fetchall()  # return list of tuples
        # need to get the header?? how to get full data from SQL
        self.hist_div_df = pandas.DataFrame(rows, columns=headers)
    ...
    def extr_hist_price_by_date(self, date_interval):
        ...
        self.hist_price_df = self.hist_price_df[self.hist_price_df['Datekey']
                                                >= target_datekey]
