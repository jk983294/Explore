...
public:
    ...
    void start() {
        room_.join(shared_from_this());  // when join, accept recent msgs
        do_read_header();                // then read new msgs
    }

    void deliver(const chat_message& msg) {
        bool write_in_progress = !write_msgs_.empty();
        write_msgs_.push_back(msg);
        if (!write_in_progress) {  // if first deliver, then launch it
            do_write();
        }
    }
...
private:
    ...
    void do_read_body() {
        ...
        boost::asio::async_read(socket_, boost::asio::buffer(read_msg_.body(), read_msg_.body_length()),
                                [this, self](boost::system::error_code ec, std::size_t /*length*/) {
                                    if (!ec) {
                                        room_.deliver(read_msg_);  // will diliver to other clients in the room
                                        do_read_header();
                                    } else {
                                        room_.leave(shared_from_this());
                                    }
...
private:
    ...
    tcp::socket socket_;
    chat_room room_;  // one server one room
};
